...
void detKeypointsShiTomasi(vector<cv::KeyPoint> &keypoints, cv::Mat &img, int img_index, std::string descriptorType,bool bVisDetect)
{
    ...
    report <<"SHITOMASI,"<< descriptorType<< ","<<img_index <<","<<keypoints.size() <<","<<1000 * t / 1.0 ;
    report.close();
...
void detKeypointsHarris(std::vector<cv::KeyPoint> &keypoints, cv::Mat &img, int img_index ,std::string descriptorType,bool bVisDetect) {
    ...
    fstream report;
    report.open("../output/Matching_Descriptors_report.txt", std::ios_base::app);
    report <<"HARRIS," << descriptorType<< ","<<img_index <<","<<keypoints.size() <<","<<1000 * t / 1.0;
    report.close();  
...
void descKeypoints(vector<cv::KeyPoint> &keypoints, cv::Mat &img, cv::Mat &descriptors, string descriptorType , string detectorType , int img_index , bool bVisDescr)
{
    ...
    fstream report;
    report.open("../output/Matching_Descriptors_report.txt", std::ios_base::app);
    report << keypoints.size() <<","<<1000 * t / 1.0;
    report.close();   
                      ...
                      std::vector<cv::DMatch> &matches, std::string descriptorType, std::string matcherType, std::string selectorType, std::vector<DataFrame> dataBuffer, std::string detector, std::string descriptor,int img_index,bool bVisMatch )
{
    ...
    fstream report;
    report.open("../output/Matching_Descriptors_report.txt", std::ios_base::app);
    report <<","<< matches.size() << "," << 1000 * t / 1.0 <<","<<KNN_keypoints_removed<<'\n';
    report.close();
